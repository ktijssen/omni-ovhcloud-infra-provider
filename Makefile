...
#
# Generated on 2026-08-21T07:01:19Z by kres 063d4c2.
...
all: unit-tests omni-infra-provider-ovhcloud image-omni-infra-provider-ovhcloud lint
...
$(ARTIFACTS)/omni-infra-provider-ovhcloud-linux-amd64:
	@$(MAKE) local-omni-infra-provider-ovhcloud-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: omni-infra-provider-ovhcloud-linux-amd64
omni-infra-provider-ovhcloud-linux-amd64: $(ARTIFACTS)/omni-infra-provider-ovhcloud-linux-amd64  ## Builds executable for omni-infra-provider-ovhcloud-linux-amd64.

.PHONY: $(ARTIFACTS)/omni-infra-provider-ovhcloud-linux-arm64
$(ARTIFACTS)/omni-infra-provider-ovhcloud-linux-arm64:
	@$(MAKE) local-omni-infra-provider-ovhcloud-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: omni-infra-provider-ovhcloud-linux-arm64
omni-infra-provider-ovhcloud-linux-arm64: $(ARTIFACTS)/omni-infra-provider-ovhcloud-linux-arm64  ## Builds executable for omni-infra-provider-ovhcloud-linux-arm64.

.PHONY: omni-infra-provider-ovhcloud
omni-infra-provider-ovhcloud: omni-infra-provider-ovhcloud-linux-amd64 omni-infra-provider-ovhcloud-linux-arm64  ## Builds executables for omni-infra-provider-ovhcloud.
...
image-omni-infra-provider-ovhcloud:  ## Builds image for omni-infra-provider-ovhcloud.
	@$(MAKE) registry-$@ IMAGE_NAME="omni-infra-provider-ovhcloud"

.PHONY: sign-image-omni-infra-provider-ovhcloud
sign-image-omni-infra-provider-ovhcloud:  ## Signs the image for omni-infra-provider-ovhcloud. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/omni-infra-provider-ovhcloud:$(IMAGE_TAG)

.PHONY: rekres-no-workflows
rekres-no-workflows:
	@rm -rf .kres-workflows-backup && cp -r .github/workflows .kres-workflows-backup
	@$(MAKE) rekres
	@rm -rf .github/workflows && mv .kres-workflows-backup .github/workflows
